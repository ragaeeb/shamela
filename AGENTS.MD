...
├── testing/                      # External test suites
│   ├── e2e.test.ts               # E2E test suite (requires network)
│   └── exports.test.ts           # Build-output validation (runs post-build)
...
| `src/transform.ts` | Data transformation and denormalization utilities |
...
| `src/utils/network.ts` | HTTP utilities (`buildUrl`, `httpsGet`) with auth support |
| `src/utils/logger.ts` | Configurable internal logging system |

### Data Transformation Utilities

The `src/transform.ts` module provides utilities for converting raw database rows into rich, nested JavaScript objects:

| Function | Purpose |
|----------|---------|
| `denormalizeBooks` | Resolves the relationships in `MasterData` to return an array of books with their authors and categories attached |

### Span Handling Utilities

Shamela HTML content uses `<span>` tags with special attributes to mark titles, subtitles, and other semantic elements. The `src/content.ts` module exports utilities for parsing and normalizing these spans:

| Function | Purpose |
|----------|---------|
| `parseContentRobust` | Parses HTML into `Line[]` objects, extracting title IDs from `<span data-type="title" id="toc-X">` |
| `normalizeHtml` | Converts `<hadeeth-N>` to `<span class="hadeeth">` for CSS styling |
| `htmlToMarkdown` | Converts `<span data-type="title">` to `## ` Markdown headers |
| `normalizeTitleSpans` | Handles consecutive title spans (merge, splitLines, or hierarchy strategies) |
| `moveContentAfterLineBreakIntoSpan` | Moves pre-title text (e.g., chapter numbers) into the span |
| `convertContentToMarkdown` | Full pipeline: normalize spans → move pre-title text → convert to Markdown |
| `removeTagsExceptSpan` | Strips `<a>` and `<hadeeth>` tags while preserving `<span>` elements |
| `stripHtmlTags` | Removes all HTML tags, keeping only text content |
| `normalizeLineEndings` | Normalizes `\r\n` and `\r` to `\n` for consistent processing |

#### Span Patterns in Shamela Content

- **Title spans**: `<span data-type="title" id="toc-123">Chapter Title</span>`
- **Consecutive titles**: Multiple adjacent title spans (handled by `normalizeTitleSpans`)
- **Hadeeth tags**: `<hadeeth-N>` opening tags with `</hadeeth>` closing tags
- **Narrator links**: `<a href="inr://...">narrator name</a>` (stripped by `htmlToMarkdown`)
...
| `shamela/transform` | `src/transform.ts` | Data transformation and denormalization |
...
# Run build-output validation (manually)
bun run test:exports
```

### Build Validation

The library includes a strict build-output validation test in `testing/exports.test.ts`. This test:
1. Runs automatically via the `postbuild` script after every successful `bun run build`.
2. Uses `tsc --noEmit` to verify that all documented types are correctly exported and usable from the `dist` bundle.
3. Uses `bun test` to verify that all functions and constants are physically present in the compiled JavaScript.

This prevents regression errors where a package entry point might compile but fail to expose the expected public API.
